...
## Foundation
- [ ] T001: Create core/store.py — EventStore with SQLite + FTS, source-agnostic schema (events table with source, account, channel, event_id, event_type, actor, title, body, metadata, processed flag)
- [ ] T002: Create core/brain.py — LLM analyzer using claude -p, builds context prompt from EventStore, returns action decisions (IGNORE/LOG/RESPOND/DISPATCH/ALERT)
